...
| 오케스트레이터        | 예정           | 도메인 에이전트 라우팅 + 멀티 에이전트 협업     |
...
데이터 수집·도메인 로직·Slack 응답 처리가 시스템의 대부분이라, 60가지 이상의 메트릭 스키마와 fast path 명령 라우팅을 타입으로 안전하게 관리할 수 있는 점을 우선했다. Slack Bolt가 TypeScript 1급 지원.
