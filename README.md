...
확장 가능한 도메인 분리 구조로 설계했다. 각 도메인 에이전트가 오케스트레이터를 통해 협업하며 운영을 처리하는 구조를 목표로, 단계적으로 완성해나가는 중. 현재 **데이터 분석·매출** 두 도메인이 운영 중이고, 나머지 세 도메인(**콘텐츠·스토어 운영·오케스트레이터**)은 앞으로 진행할 예정이다.
...
| 콘텐츠                | 예정           | 사진 라이브러리 자동 분석·태깅 → 어떤 사진·메시지를 SNS에 쓸지 마케팅 콘텐츠 제안 |
| 스토어&nbsp;운영      | 예정           | 데이터 분석 에이전트 제안을 받아 검색 태그·상품 정보 등 스토어 설정 직접 수정 |
| 오케스트레이터        | 예정           | 도메인 에이전트 라우팅 + 협업 처리              |
...
| 영역       | 선택                                                          |
| ---------- | ------------------------------------------------------------- |
| AI/LLM     | Claude Opus 4.7 (Claude 앱 Scheduled Task로 호출)             |
